...
function Navigation({ isLoaded }) {
    ...
    useEffect(() => {
        const innerNavDiv = document.getElementsByClassName('inner-nav-div')[0];
        const searchResultsPageContainer = document.getElementsByClassName('search-results-page-container')[0];
        if(innerNavDiv && searchParams) {
            innerNavDiv.setAttribute('id', 'search-results-top-nav');
        } else {
            innerNavDiv.removeAttribute('id');
        }
    }, [searchParams])
